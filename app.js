...
app.get('/api/ltv', (req, res) => {
    if (!req.session.username) {
        return res.status(401).json({ success: false, message: 'Unauthorized' });
    }

    const query = `
        SELECT producto, year, value, \`show\`
        FROM ltv
    `;
    ...
    db.query(query, (err, results) => {
        ...
        const ltvData = { productos: {} };
        results.forEach(row => {
            const productoNombre = row.producto;
            if (!ltvData.productos[productoNombre]) {
                ltvData.productos[productoNombre] = {};
            }
            ltvData.productos[productoNombre][row.year] = {
                value: row.value,
                show: row.show
            };
        });
...
app.get('/api/calculadora', (req, res) => {
    ...
    const query = `
        SELECT p.nombre AS producto, p.plazo, p.interest, p.fee, p.minfee, c.minAFinanciar
        FROM productos p
        LEFT JOIN configuracion c ON 1=1
    `;
    ...
    db.query(query, (err, results) => {
        if (err) {
            console.error('Error fetching calculadora data:', err);
            return res.status(500).json({ success: false, message: 'Internal Server Error' });
        }

        const calculadoraData = { productos: {}, minAFinanciar: null };
        results.forEach(row => {
            const productoNombre = row.producto;
            if (!calculadoraData.productos[productoNombre]) {
                calculadoraData.productos[productoNombre] = { plazos: {} };
            }
            calculadoraData.productos[productoNombre].plazos[row.plazo] = {
                interest: row.interest,
                fee: row.fee,
                minfee: row.minfee
            };
            if (row.minAFinanciar !== null) {
                calculadoraData.minAFinanciar = row.minAFinanciar;
            }
        });

        res.json(calculadoraData);
    });
});
